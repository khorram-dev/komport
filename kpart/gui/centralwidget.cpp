...
void CentralWidget::loadState(KSharedConfigPtr cfg)
{
    ...
    Activity* act = static_cast<Activity*>(widget(idx));
    if (act)
    ...
    for (QAction* a : actions)
    {
        a->setPriority( static_cast<QAction::Priority>(g.readEntry(QLatin1String("Priority_") + a->objectName(), static_cast<int>(QAction::NormalPriority))));
    }
}
...
void CentralWidget::saveState(KSharedConfigPtr cfg)
{
    ...
    {
        g.writeEntry(QLatin1String("Priority_") + a->objectName(), static_cast<int>(a->priority()));
    }
}
...
{
    return static_cast<Activity*>(currentWidget());
}
...
void CentralWidget::switchActivity(QAction* action)
{
    ...
    {
        Activity* act = static_cast<Activity*>(widget(i));
