/*  SPDX-License-Identifier: LGPL-2.0-or-later
    ...
    Changes: 2019 by Masoud Khorram <masoudkh62@gmail.com>
*/
...
static const QString actionListName = QStringLiteral("Komport_mdi_view_actions");
...
static const QString guiDescription = QStringLiteral(
    ""
    "<!DOCTYPE kpartgui><kpartgui name=\"Komport_mdi_view_actions\">"
    "<MenuBar>"
    "    <Menu name=\"view\">"
    "        <ActionList name=\"%1\" />"
    "    </Menu>"
    ...
    , m_mw(mw)
{
    ...
    actionCollection()->addAction(QStringLiteral("Komport_mdi_toolview_menu"), m_toolMenu);
    m_showSidebarsAction = new KToggleAction(i18n("Show Side&bars"), this);
    actionCollection()->addAction(QStringLiteral("Komport_mdi_sidebar_visibility"), m_showSidebarsAction);
...
{
    QString aname = QLatin1String("Komport_mdi_toolview_") + tv->id;
...
void Sidebar::buttonPopupActivate(QAction *a)
{
    ...
    if (id == 20) {
        if (!w->widget.isNull()) {
            if (w->widget.isNull() > 0) {
                emit sigShowPluginConfigPage(w->widget, 0);
            }
        }
    }
}
...
void Sidebar::restoreSession(KConfigGroup &config)
{
    ...
    for (; firstWrong < m_toolviews.size(); ++firstWrong) {
        ...
        int pos = config.readEntry(QStringLiteral("Komport-MDI-ToolView-%1-Sidebar-Position").arg(tv->id), firstWrong);
    ...
    if (firstWrong < m_toolviews.size()) {
        ...
        for (int i = firstWrong; i < m_toolviews.size(); ++i) {
            TmpToolViewSorter s;
            s.tv = m_toolviews[i];
            s.pos = config.readEntry(QStringLiteral("Komport-MDI-ToolView-%1-Sidebar-Position").arg(m_toolviews[i]->id), i);
            toSort.push_back(s);
        }
    ...
    QList<int> s = config.readEntry(QStringLiteral("Komport-MDI-Sidebar-%1-Splitter").arg(position()), QList<int>());
    m_ownSplit->setSizes(s);

    // show only correct toolviews, remember persistent values ;)
    bool anyVis = false;
    for (auto tv : qAsConst(m_toolviews)) {
        tv->persistent = config.readEntry(QStringLiteral("Komport-MDI-ToolView-%1-Persistent").arg(tv->id), false);
        tv->setToolVisible(config.readEntry(QStringLiteral("Komport-MDI-ToolView-%1-Visible").arg(tv->id), false));
...
void Sidebar::saveSession(KConfigGroup &config)
{
    ...
    config.writeEntry(QStringLiteral("Komport-MDI-Sidebar-%1-Splitter").arg(position()), s);

    // store the data about all toolviews in this sidebar ;)
    for (int i = 0; i < m_toolviews.size(); ++i) {
        ToolView *tv = m_toolviews[i];

        config.writeEntry(QStringLiteral("Komport-MDI-ToolView-%1-Position").arg(tv->id), int(tv->sidebar()->position()));
        config.writeEntry(QStringLiteral("Komport-MDI-ToolView-%1-Sidebar-Position").arg(tv->id), i);
        config.writeEntry(QStringLiteral("Komport-MDI-ToolView-%1-Visible").arg(tv->id), tv->toolVisible());
        config.writeEntry(QStringLiteral("Komport-MDI-ToolView-%1-Persistent").arg(tv->id), tv->persistent);
    }
}
...
MainWindow::MainWindow() : m_guiClient(new GUIClient(this))
{
...
ToolView *MainWindow::createToolView(QWidget *widget, const QString &identifier, KMultiTabBar::KMultiTabBarPosition pos, const QIcon &icon, const QString &text)
{
    ...
    if (m_restoreConfig && m_restoreConfig->hasGroup(m_restoreGroup)) {
        ...
        pos = static_cast<KMultiTabBar::KMultiTabBarPosition>(cg.readEntry(QStringLiteral("Komport-MDI-ToolView-%1-Position").arg(identifier), int(pos)));
    }
...
void MainWindow::setSidebarsVisible(bool visible)
{
    ...
    if (old_visible && (!m_sidebarsVisible)) {
        KMessageBox::information(this,
                                 ...
                                 QString(),
                                 QStringLiteral("Komport hide sidebars notification message"));
    }
}
...
bool MainWindow::moveToolView(ToolView *widget, KMultiTabBar::KMultiTabBarPosition pos)
{
    ...
    if (m_restoreConfig && m_restoreConfig->hasGroup(m_restoreGroup)) {
        KConfigGroup cg(m_restoreConfig, m_restoreGroup);
        pos = static_cast<KMultiTabBar::KMultiTabBarPosition>(cg.readEntry(QStringLiteral("Komport-MDI-ToolView-%1-Position").arg(widget->id), int(pos)));
    }
...
void MainWindow::startRestore(KConfigBase *config, const QString &group)
{
    ...
    setToolViewStyle(static_cast<KMultiTabBar::KMultiTabBarStyle>(cg.readEntry("Komport-MDI-Sidebar-Style", static_cast<int>(toolViewStyle()))));
    // after reading m_sidebarsVisible, update the GUI toggle action
    m_sidebarsVisible = cg.readEntry("Komport-MDI-Sidebar-Visible", true);
    m_guiClient->updateSidebarsVisibleAction();
}
...
void MainWindow::finishRestore()
{
    ...
    if (m_restoreConfig->hasGroup(m_restoreGroup)) {
        ...
        for (const auto tv : qAsConst(m_toolviews)) {
            KMultiTabBar::KMultiTabBarPosition newPos = static_cast<KMultiTabBar::KMultiTabBarPosition>(cg.readEntry(QStringLiteral("Komport-MDI-ToolView-%1-Position").arg(tv->id), int(tv->sidebar()->position())));
        ...
        hs = cg.readEntry("Komport-MDI-H-Splitter", hs);
        vs = cg.readEntry("Komport-MDI-V-Splitter", vs);
...
void MainWindow::saveSession(KConfigGroup &config)
{
    ...
    config.writeEntry("Komport-MDI-H-Splitter", hs);
    config.writeEntry("Komport-MDI-V-Splitter", vs);

    // save sidebar style
    config.writeEntry("Komport-MDI-Sidebar-Style", static_cast<int>(toolViewStyle()));
    config.writeEntry("Komport-MDI-Sidebar-Visible", m_sidebarsVisible);
